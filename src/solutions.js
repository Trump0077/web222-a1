...
function createIframe(src, height, allowFullScreen) {
  ...
  var newHeight = '';
  var newAllowFullScreen = '';
  height = Number(height);
  if (height > 0 && Number.isInteger(height)) {
    newHeight = ` height="${height}"`;
  }
  if (allowFullScreen) {
    newAllowFullScreen = ' allowfullscreen';
  }

  return `<iframe src="${trimSrc}"${newHeight}${newAllowFullScreen}></iframe>`;
}
/*******************************************************************************
...
function fixPostalCode(postalCode) {
  ...
  throw `error:${fixedPostalCode} is not valid`;
}
...
function toProvince(postalCode, useShortForm) {
  var capPostalCode = postalCode.toUpperCase();
  var newPostalCode = capPostalCode.replace(/\s/, '');
  var letter1 = newPostalCode.substring(0, 1);

  if (newPostalCode.substring(0, 3).match(/[A-Z]\d[A-Z]/gim) && newPostalCode.length === 6) {
    let text1 = 'KLMNP';
    if (text1.includes(letter1)) {
      if (useShortForm) {
        return 'ON';
      }
      return 'Ontario';
...
function normalizeCoord(value) {
  ...
  if (lat < -90 || lat > 90 || lng < -180 || lng > 180) throw 'Invalid Coordinates';
...
function formatCoords(...values) {
  ...
  for (var i = 0; i < arguments.length; i++) {
    try {
      if (coords !== '(') coords += ', ' + normalizeCoord(arguments[i]);
      else coords += normalizeCoord(arguments[i]);
    } catch (err) {
...
function generateLicenseLink(licenseCode, targetBlank) {
  let licenseNewCode;
  let textNew;
  let targetBlankNew = targetBlank ? ` target="_blank"` : '';

  if (licenseCode === 'CC-BY') {
    textNew = 'Creative Commons Attribution License';
    licenseNewCode = 'https://creativecommons.org/licenses/by/4.0/';
  } else if (licenseCode === 'CC-BY-NC') {
    textNew = 'Creative Commons Attribution-NonCommercial License';
    licenseNewCode = 'https://creativecommons.org/licenses/by-nc/4.0/';
  } else if (licenseCode === 'CC-BY-SA') {
    textNew = 'Creative Commons Attribution-ShareAlike License';
    licenseNewCode = 'https://creativecommons.org/licenses/by-sa/4.0/';
  } else if (licenseCode === 'CC-BY-ND') {
    textNew = 'Creative Commons Attribution-NoDerivs License';
    licenseNewCode = 'https://creativecommons.org/licenses/by-nd/4.0/';
  } else if (licenseCode === 'CC-BY-NC-SA') {
    textNew = 'Creative Commons Attribution-NonCommercial-ShareAlike License';
    licenseNewCode = 'https://creativecommons.org/licenses/by-nc-sa/4.0/';
  } else if (licenseCode === 'CC-BY-NC-ND') {
    textNew = 'Creative Commons Attribution-NonCommercial-NoDerivs License';
    licenseNewCode = 'https://creativecommons.org/licenses/by-nc-nd/4.0/';
  } else {
    textNew = 'All Rights Reserved';
    licenseNewCode = 'https://choosealicense.com/no-permission/';
  }
  return `<a href="${licenseNewCode}"${targetBlankNew}>${textNew}</a>`;
}
...
function pureBool(value) {
  if (typeof value === 'boolean') {
    ...
    let uppCaseValue = value.toUpperCase();
    let arrPositive = ['YES', 'Y', 'OUI', 'O', 'TRUE', 'T', 'VRAI', 'V'];
    let arrNegative = ['NO', 'N', 'NON', 'FALSE', 'F', 'FAUX'];
    if (arrPositive.includes(uppCaseValue)) {
      return true;
    }
    if (arrNegative.includes(uppCaseValue)) {
      return false;
    }
  } else {
    throw 'invalid value';
...
function buildUrl(query, order, count, license) {
  ...
  return `https://api.inaturalist.org/v2/observations?query=${encodeQuery}&count=${count}&order=${order}&license=${license}`;
}
